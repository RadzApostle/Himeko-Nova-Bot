  ...
  conn.game[id] = [
    ...
    setTimeout(() => {
      if (conn.game[id]) {
        ...
        const _ref = conn.game; delete _ref[id]
      }
    }, timeout)
  ]
}
...
handler.before = async function (m, { conn }) {
  ...
  if (teks === jawaban) {
    clearTimeout(time)
    const _ref = conn.game; delete _ref[id]
    if (global.db.data.users[m.sender]) global.db.data.users[m.sender].money += hadiah
    if (global.db.data.users[m.sender]) global.db.data.users[m.sender].limit += 1
    m.reply(`*🎉 BENAR! 🎉*\n+${hadiah} 💰Money\n+1 🎫Limit`)
    return true
  }
